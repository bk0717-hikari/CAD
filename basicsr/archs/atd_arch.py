...
def index_reverse(index):#生成一个反向索引，用于还原打乱后的特征顺序
...
def feature_shuffle(x, index):#根据索引打乱特征张量
    dim = index.dim()
...
#深度卷积层
class dwconv(nn.Module):#实现了一个基于深度卷积的模块
    def __init__(self, hidden_features, kernel_size=5):
        ...
        self.depthwise_conv = nn.Sequential(
            nn.Conv2d(hidden_features, hidden_features, kernel_size=kernel_size, stride=1, padding=(kernel_size - 1) // 2, dilation=1,
                      groups=hidden_features), nn.GELU())#使用组卷积，限制了每个通道只与自身进行卷积计算
...
#包含卷积的前馈神经网络
class ConvFFN(nn.Module):#结合线性层和卷积的前馈神经网络
...
#图像块分割
def window_partition(x, window_size):#将图像分割成窗口大小的子块
    """
    Args:
...
#窗口注意力机制
class WindowAttention(nn.Module):#这是一个基于窗口的多头自注意力机制，每个窗口单独进行注意力计算，降低了全局自注意力的计算复杂度
    r"""
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True):
        ...
        self.qkv_bias = qkv_bias#是否在q,k,v中添加偏置
        head_dim = dim // num_heads
        self.scale = head_dim**-0.5#用于缩放查询向量，防止内积值过大导致 softmax 的梯度过小

        # define a parameter table of relative position bias
        self.relative_position_bias_table = nn.Parameter(#相对位置偏差表，用于表示窗口中不同位置之间的相对位置关系
            torch.zeros((2 * window_size[0] - 1) * (2 * window_size[1] - 1), num_heads))  # 2*Wh-1 * 2*Ww-1, nH

        self.proj = nn.Linear(dim, dim)#定义一个线性投影层，用于将输出特征重新映射到输入通道数

        trunc_normal_(self.relative_position_bias_table, std=.02)
        self.softmax = nn.Softmax(dim=-1)#定义 softmax 层，用于计算注意力权重
    ...
    def forward(self, qkv, rpi, mask=None):
        ...
        attn = (q @ k.transpose(-2, -1))#计算注意力得分矩阵

        relative_position_bias = self.relative_position_bias_table[rpi.view(-1)].view(
            self.window_size[0] * self.window_size[1], self.window_size[0] * self.window_size[1], -1)  # Wh*Ww,Wh*Ww,nH
        relative_position_bias = relative_position_bias.permute(2, 0, 1).contiguous()  # nH, Wh*Ww, Wh*Ww
        attn = attn + relative_position_bias.unsqueeze(0)#添加相对位置偏差
    ...
    def flops(self, n):
        ...
        flops += self.num_heads * n * (self.dim // self.num_heads) * n#计算查询与键相乘的计算量
        #  x = (attn @ v)
        flops += self.num_heads * n * n * (self.dim // self.num_heads)#计算值与注意力分数相乘的计算量
        # x = self.proj(x)
        flops += n * self.dim * self.dim#计算线性投影的计算量
        return flops
...
class ATD_CA(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_tokens=64, reducted_dim=10, qkv_bias=True):
        ...
        self.num_tokens = num_tokens#外部令牌字典的令牌数量
        self.rc = reducted_dim#缩减后的维度，用于降低计算复杂度
        ...
        self.scale = nn.Parameter(torch.ones([self.num_tokens]) * 0.5, requires_grad=True)#初始化为每个令牌的缩放因子，初始值为 0.5，该参数可在训练中学习优化
...
class AC_MSA(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_tokens=64, num_heads=4, category_size=128, qkv_bias=True):
        ...
        self.num_heads = num_heads
        self.category_size = category_size#每组的最大特征数量，用于全局稀疏注意力的分组操作
    ...
    def forward(self, qkv, sim, x_size):
        ...
        # 打印调试信息
        #print(f"batch size: {b}, num groups: {ng}, group size: {gs}, num_heads: {self.num_heads}, channels per head: {c // self.num_heads}")

        # 检查 channels 是否能被 num_heads 整除
        ...
        tk_id = torch.argmax(sim, dim=-1, keepdim=False)#根据 sim 将每个特征分配到最相似的类别
...
class ATDTransformerLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.is_last = is_last#是否为 ATD 块的最后一层

        self.norm1 = norm_layer(dim)
        self.norm2 = norm_layer(dim)#对输入进行归一化，提升训练稳定性
        if not is_last:
    ...
    def forward(self, x, td, x_size, params):
        ...
        if self.shift_size > 0:
            ...
            attn_x = shifted_x
        x_win = attn_x
        x = shortcut + x_win.view(b, n, c) + x_atd + x_aca#将 SW-MSA、ATD_CA 和 AC-MSA 的输出特征叠加在残差连接上

        # FFN
        x = x + self.convffn(self.norm2(x), x_size)#对融合后的特征进行前馈增强
        ...
        return x, td
    """
        通过令牌特征 td 和特征 x 更新令牌。
        使用 sim_atd 的相似性映射计算权重。
        更新后的令牌结合旧令牌和新提取特征
    """
...
class ATDB(nn.Module):
    ...
    def __init__(self,
        ...
        self.dim = dim
        self.input_resolution = input_resolution
        self.window_size = window_size# 将 window_size 初始化为类属性
...
class ATD(nn.Module):
     ...
     def __init__(self,
        ...
        self.conv_first = nn.Conv2d(num_in_ch, embed_dim, 3, 1, 1)#浅层特征提取
        ...
        self.patch_embed = PatchEmbed(
            ...
            in_chans=embed_dim,
            embed_dim=embed_dim,
            norm_layer=norm_layer if self.patch_norm else None)#将输入图像划分为固定大小的patch，并线性嵌入到高维特征空间中
        ...
        self.patch_unembed = PatchUnEmbed(
            ...
            in_chans=embed_dim,
            embed_dim=embed_dim,
            norm_layer=norm_layer if self.patch_norm else None)#将高维重新映射回图像空间中
     ...
     def forward_features(self, x, params):
        x_size = (x.shape[2], x.shape[3])#高度和宽度
